    ...
    <div className="bg-white rounded-xl p-6 border border-[#E2E8F0] mb-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div>  <span className="text-lg font-semibold text-[#0F172A]">
              Forecast Selection for product:
            </span>
            <span className="text-xl font-bold ms-1">{productName.toUpperCase()}</span></div>
          
            <p className="text-sm text-[#64748B]">
              Choose which forecast to display and analyze for product: {productName.toUpperCase()}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm text-[#64748B]">
            {allForecasts.length} forecast{allForecasts.length !== 1 ? 's' : ''} available for product: {productName.toUpperCase()}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
            ...
            <SelectContent>
              <SelectItem value="latest">
                <div className="flex items-center gap-2">
                  ...
                  <span>Latest Forecast for product: {productName.toUpperCase()}</span>
                </div>
              </SelectItem>
              <SelectItem value="all">
                <div className="flex items-center gap-2">
                  <List className="h-4 w-4 text-blue-600" />
                  <span>View All Forecasts for product: {productName.toUpperCase()}</span>
                </div>
              </SelectItem>
                ...
                <SelectItem key={forecast.id} value={forecast.id}>
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {formatDate(forecast.createdAt)} for product: {productName.toUpperCase()}
                      </span>
                    </div>
        ...
        {selectedForecast.type === 'specific' && (
          <div className="flex items-center gap-2 text-sm text-[#64748B] bg-blue-50 px-3 py-2 rounded-lg">
            ...
            <span>Viewing specific forecast for {productName}</span>
          </div>
        )}
      </div>
      ...
      {selectedForecast.type === 'all' && (
        <div className="mt-6 border-t border-[#E2E8F0] pt-6">
          <h4 className="font-semibold text-[#0F172A] mb-4 flex items-center gap-2">
            ...
            All Generated Forecasts for {productName}
          </h4>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-medium text-[#0F172A]">
                      {productName} Forecast from {formatDate(forecast.createdAt)}
                    </p>
                    <p className="text-sm text-[#64748B]">
          ...
          {forecastPagination.totalPages > 1 && (
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-[#E2E8F0]">
              <p className="text-sm text-[#64748B]">
                Showing {allForecasts.length} of {forecastPagination.totalForecasts} forecasts for {productName}
              </p>
              <div className="flex items-center gap-2">
                <button
