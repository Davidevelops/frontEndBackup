...
import { Sale, ProductGroup } from "@/lib/types"
import {
  ShoppingCart,
  TrendingUp,
  AlertCircle,
...
// Define the props interface with groupName
...
export default function SalesTable({ 
  ...
  productId, 
  onRefetch,
  groupName = "" // Add default value here
}: SalesTableProps) {
  ...
  const fetchProducts = async (): Promise<ProductMap> => {
    setLoadingProducts(true)
    try {
      const productGroups = await getProductList()
      
      if (!productGroups) {
        logger.warn('No product groups found')
        return {}
      }
      ...
      productGroups.forEach((group: ProductGroup) => {
        group.products?.forEach((product: SingleProduct) => {
          map[product.id] = {
            name: product.name || `Product ${product.id.slice(0, 8)}`,
            groupName: group.name || `Group ${group.id.slice(0, 8)}`,
            groupId: group.id
          }
        })
      })

      logger.info('Product map created', { 
  ...
  const fetchAllData = useCallback(async (retryCount = 0): Promise<void> => {
    const maxRetries = 3
    ...
    try {
      setLoading(true)
      setFetchError(null)
      logger.info('Fetching product data', { retryCount })
      ...
      if (productData) {
        ...
        logger.warn('Product data fetch failed, using IDs only')
        setProductMap({})
      }

    } catch (error: any) {
      const errorContext = `fetchAllData (attempt ${retryCount + 1})`
      
      if (retryCount < maxRetries) {
        logger.warn(`Retrying fetch after ${retryDelay}ms`, { retryCount })
        setTimeout(() => fetchAllData(retryCount + 1), retryDelay)
        return
      }
      ...
      handleError(error, errorContext, 'Failed to load product data')
  ...
  const getProductDisplayName = useCallback((productId: string) => {
  ...
  const getProductGroupName = useCallback((productId: string) => {
    if (!productId) return "Unknown Group"
    
    const productInfo = productMap[productId]
    
    if (productInfo) {
      return productInfo.groupName
    }
    
    return "N/A"
  }, [productMap])

  const getProductGroupId = useCallback((productId: string) => {
    if (!productId) return ""
    
    const productInfo = productMap[productId]
    
    if (productInfo) {
      return productInfo.groupId
    }
  ...
  const enrichedSales = useMemo(() => {
    ...
    return sales.map(sale => ({
      ...sale,
      productName: getProductDisplayName(sale.productId),
      groupName: getProductGroupName(sale.productId),
      groupId: getProductGroupId(sale.productId)
    }))
  }, [sales, getProductDisplayName, getProductGroupName, getProductGroupId])
            ...
            <div className="animate-pulse flex flex-col items-center">
              ...
              {loadingProducts && (
                <div className="mt-4 flex items-center gap-2">
                  <div className="bg-[#E2E8F0] h-3 w-3 rounded-full animate-pulse"></div>
                  <div className="bg-[#E2E8F0] h-3 w-24 rounded"></div>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="max-w-[95rem] mx-auto">
        {/* Header Section - Show current variant if on variant-specific page */}
        {(productName && groupId && productId) && (
                  ...
                  {groupName ? (
                    <>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <ChevronRightIcon className="h-4 w-4 text-[#CBD5E1] mx-1" />
            ...
            <div>
              ...
              {Object.keys(productMap).length > 0 && (
                <p className="text-sm text-green-600 mt-2">
                  ✓ Product names loaded: {Object.keys(productMap).length} products across {new Set(Object.values(productMap).map(p => p.groupId)).size} groups
                </p>
              )}
            </div>
            <div className="flex items-center gap-4">
          ...
          {sales === null ? (
            ...
            <div className="text-center py-20">
              <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-10 max-w-md mx-auto">
                ...
                <label className="flex items-center justify-center gap-2 bg-green-100 text-xs text-green-700 hover:bg-green-200 border border-green-400 px-4 py-2.5 rounded-lg transition-all duration-200 font-medium cursor-pointer">
                  ...
                  <p className="text-sm text-[#64748B] mt-1">
                    ...
                    {Object.keys(productMap).length === 0 && (
                      <span className="ml-2 text-amber-600">
                        (Product names not available)
                      </span>
                    )}
                  </p>
                </div>
                {Object.keys(productMap).length > 0 && (
                  <div className="mt-2 sm:mt-0 flex items-center gap-2 text-sm text-green-600 bg-green-50 px-3 py-1.5 rounded-lg">
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-[#E2E8F0]">
                    {paginatedSales.map((sale) => {
                      const productInfo = productMap[sale.productId]
                      const productDisplayName = getProductDisplayName(sale.productId)
                        ...
                        >
                          ...
                          <td className="py-4 px-4">
                            <div className="flex flex-col space-y-1">
                              ...
                              {/* Group Name */}
                              {productInfo && productInfo.groupName && (
                                <div className="text-xs text-[#64748B] flex items-center gap-1">
                                  <Layers className="h-3 w-3" />
                                  <span>{productInfo.groupName}</span>
                                </div>
                              )}
                              {/* Product ID (only if different from current product) */}
                              {(productId && sale.productId !== productId) && (
                                <div className="text-xs font-mono text-[#94A3B8]" title={sale.productId}>
                                  ID: {sale.productId.slice(0, 8)}...
                                </div>
                              )}
                            </div>
                          </td>
