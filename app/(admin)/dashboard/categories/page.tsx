  ...
  export default function CategoriesPage() {
    ...
    const fetchCategoriesData = async () => {
      try {
        console.log("🔄 Fetching categories data with groups...")
        const categoriesData = await getCategories(true)
        if (categoriesData) {
          console.log("Categories data received:", categoriesData)
          categoriesData.forEach(category => {
            console.log(`Category ${category.name} has ${category.productGroups?.length || 0} product groups`);
          });
    ...
    const handleAssignProductGroup = async (categoryId: string, groupId: string) => {
      ...
      try {
        ...
        if (selectedCategory && selectedCategory.id === categoryId) {
          // Use getCategory with includeGroups set to true
            ...
            {categories === null ? (
              <div className="text-center py-20">
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-8 max-w-md mx-auto shadow-xs">
                  ...
                  <h3 className="text-lg font-semibold text-slate-800 mb-2">Failed to Load Categories</h3>
                  <p className="text-slate-600 mb-4">There was an error loading the categories. Please try again.</p>
                  <button
