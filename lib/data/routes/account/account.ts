...
export const createAccount = async (accountData: CreateAccountRequest) => {
  try {
    const response = await axiosInstance.post(apiEndpoints.account(), accountData, {
      headers: {
...
export const getAvailablePermissions = async (): Promise<Permission[] | null> => {
  try {
    const response = await axiosInstance.get(apiEndpoints.permissions());
    return response.data.data;
  } catch (error) {
...
export const updateAccount = async (accountId: string, accountData: { username: string; role: string }) => {
  try {
    const response = await axiosInstance.patch(apiEndpoints.account(accountId), accountData, {
      headers: {
        "Content-Type": "application/json",
      },
    });
...
export const deleteAccount = async (accountId: string) => {
  try {
    const response = await axiosInstance.delete(apiEndpoints.account(accountId));
    return response.data;
  } catch (error: any) {
...
export const changePassword = async (accountId: string, passwordData: { password: string }) => {
  try {
    const response = await axiosInstance.put(apiEndpoints.accountPassword(accountId), passwordData, {
      headers: {
        "Content-Type": "application/json",
      },
    });
