...
// Products Excel API
export const exportProductsTemplate = async (
  ...
  try {
    ...
    const url = `${apiEndpoints.excelProducts()}/export${
...
export const exportSalesTemplate = async (
  ...
  try {
    ...
    const url = `${apiEndpoints.excelSales()}/export${
      queryParams.toString() ? `?${queryParams.toString()}` : ""
    }`;
...
export const importSales = async (file: File): Promise<ImportResult> => {
  try {
    ...
    const response = await axios.post(
      `${apiEndpoints.excelSales()}/import`,
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
